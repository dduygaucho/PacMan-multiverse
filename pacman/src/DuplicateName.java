...
//      filter all strings starting with numbers
...
//            sort according to first number
        ...
        for (Integer index: levelDict.keySet()){
            if (levelDict.get(index).size() > 1){
                try {
                    ...
                    fileWriter.write
                            (this.filePath + " - multiple maps at same level: "+
                                    this.convertToString(levelDict.get(index)));
                    fileWriter.write("\n");
                    fileWriter.close();
                } catch (IOException e) {
                    e.printStackTrace();
