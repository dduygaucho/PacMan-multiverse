...
//  used for constructing the maze array, which is essential for Game play
  public PacManGameGrid(String map, PortalDict protals, boolean auto){
...
//    read the xml file code
    ...
    try {
      ...
      // change the argument passed in later
      boolean isAuto = writePacmanAutoToFile(filePathProperties,auto);
...
//  no longer always returns false
  public boolean writePacmanAutoToFile(String filePath, boolean isAuto){
    try (BufferedWriter writer = new BufferedWriter(new FileWriter(filePath, true))) {
      if (isAuto == true) {
        ...
        writer.write("PacMan.isAuto=" + false + "\n" +
                "PacMan.move=\n" +
                "Pills.location=\n" +
                "Gold.location=");
        return false;
  ...
  private int toInt(char c, PortalDict portal)
  {
    ...
    if (c == 'i')
      return 4;
    if (portal.checkExistPortal("PortalYellowTile")
            && c == portal.getDefinition("PortalYellowTile")){
      return 5;
    }
    if (portal.checkExistPortal("PortalWhiteTile")
            && c == portal.getDefinition("PortalWhiteTile")){
      return 6;
    }
    if (portal.checkExistPortal("PortalDarkGoldTile")
            && c == portal.getDefinition("PortalDarkGoldTile")){
      return 7;
    }
    if (portal.checkExistPortal("PortalDarkGrayTile")
            && c == portal.getDefinition("PortalDarkGrayTile")){
      return 8;
    }
    return -1;
  }
