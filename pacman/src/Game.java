...
//  protected  PacManGameGrid grid = new PacManGameGrid("sample_map1.xml");
...
//  filePath is just a single xml file: test3.xml for instance
//  portaldict holds all values for portal strings: PortalWhileTile: 'w' for instance
//  add arguments, the left is nothing different
  public Game(GameCallback gameCallback, String filePath, PortalDict protals, boolean auto)
  {
    //Setup game
    super(nbHorzCells, nbVertCells, 20, false);
//    create a properties file in here
    grid = new PacManGameGrid(filePath, protals, auto);

//    load the properties file here
    final Properties properties
            = PropertiesLoader.loadPropertiesFile
            ("properties/test" + filePath.substring(0, filePath.length()-4) + ".properties");
...
//    System.out.println(this.properties.getProperty("Troll.location"));
//    System.out.println(this.properties.getProperty("Tx5.location"));
//    System.out.println(this.properties.getProperty("PacMan.location"));
  ...
  private void loadPillAndItemsLocations() {
    ...
    if (pillsLocationString != null) {
      ...
      for (String singlePillLocationString: singlePillLocationStrings) {
        ...
        propertyPillLocations.add
                (new Location(Integer.parseInt(locationStrings[0]), Integer.parseInt(locationStrings[1])));
      }
    }
...
//          System.out.println("YELLOW");
          putPortal(bg, location, PortalYellow);
        } else if (a == 6) {
          putPortal(bg, location, PortalWhite);
        } else if (a == 7) {
//          System.out.println("DARKGOLD");
          putPortal(bg, location, PortalDarkGold);
        } else if (a == 8) {
          putPortal(bg, location, PortalDarkGray);
        }
      }
    }
  ...
  private void putPortal(GGBackground bg, Location location, PortalType portalType) {
    bg.setPaintColor(Color.blue);
    bg.fillCircle(toPoint(location), 5);
    Portal portal = new Portal(portalType);
